...
public class RefeicaoController {
    ...
    @PostMapping
    public ResponseEntity<RefeicaoDTO> criar(@RequestBody RefeicaoInputDTO dto, Principal principal) {
        return ResponseEntity.ok(refeicaoService.criar(dto, principal.getName()));
    }

    @PutMapping("/{id}")
    public ResponseEntity<RefeicaoDTO> editar(@PathVariable Long id, @RequestBody RefeicaoInputDTO dto,
                                               Principal principal) {
        return ResponseEntity.ok(refeicaoService.editar(id, dto, principal.getName()));
    }
